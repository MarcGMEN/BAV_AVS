<script>
	function initPage() {
		...
		// x_return_list_type(display_list_type);
		...
		display_list_public(['homme','femme','mixte','enfant','garçon','fille','Autre']);
	...
	function annulerFiche(laForm) {
		if (!startSaisie || confirm("Annuler la saisie de cette fiche ?")) {
			laForm.action.value="";
			laForm.submit();
	...
	<tr>
		...
		<td width=33% align=center>
			<input type=button value="Annuler"  
				onclick="annulerFiche(this.form)"
				onkeypress="annulerFiche(this.form)" tabindex=<?=$tabindex++?> 
				></td>
